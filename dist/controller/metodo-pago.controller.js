"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MetodoPagoController = void 0;
const common_1 = require("@nestjs/common");
const metodo_pago_service_1 = require("../service/metodo-pago.service");
const metodo_pago_edit_dto_1 = require("../dto/metodo-pago-edit.dto");
const metodo_pago_create_dto_1 = require("../dto/metodo-pago-create.dto");
let MetodoPagoController = class MetodoPagoController {
    metodoPagoService;
    constructor(metodoPagoService) {
        this.metodoPagoService = metodoPagoService;
    }
    async getMetodosPago() {
        try {
            return await this.metodoPagoService.getMetodosPago();
        }
        catch (error) {
            throw new common_1.HttpException('Error al obtener métodos de pago', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async getMetodoPago(id) {
        try {
            return await this.metodoPagoService.getMetodoPago(id);
        }
        catch (error) {
            throw new common_1.HttpException('Error al obtener el método de pago por ID', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async createMetodoPago(data) {
        try {
            return await this.metodoPagoService.createMetodoPago(data);
        }
        catch (error) {
            throw new common_1.HttpException('Error al crear el método de pago', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async updateMetodoPago(id, data) {
        try {
            return await this.metodoPagoService.updateMetodoPago(data, id);
        }
        catch (error) {
            throw new common_1.HttpException('Error al actualizar el método de pago', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async deleteMetodoPago(id) {
        try {
            return await this.metodoPagoService.deleteMetodoPago(id);
        }
        catch (error) {
            throw new common_1.HttpException('Error al eliminar el método de pago', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
};
exports.MetodoPagoController = MetodoPagoController;
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], MetodoPagoController.prototype, "getMetodosPago", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], MetodoPagoController.prototype, "getMetodoPago", null);
__decorate([
    (0, common_1.Post)('crear'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [metodo_pago_create_dto_1.CreateMetodoPagoDto]),
    __metadata("design:returntype", Promise)
], MetodoPagoController.prototype, "createMetodoPago", null);
__decorate([
    (0, common_1.Put)('editar/:id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, metodo_pago_edit_dto_1.MetodoPagoEditDto]),
    __metadata("design:returntype", Promise)
], MetodoPagoController.prototype, "updateMetodoPago", null);
__decorate([
    (0, common_1.Delete)('eliminar/:id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], MetodoPagoController.prototype, "deleteMetodoPago", null);
exports.MetodoPagoController = MetodoPagoController = __decorate([
    (0, common_1.Controller)('metodo-pago'),
    __metadata("design:paramtypes", [metodo_pago_service_1.MetodoPagoService])
], MetodoPagoController);
//# sourceMappingURL=metodo-pago.controller.js.map